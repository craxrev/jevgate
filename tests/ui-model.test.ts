...
const log = [
  ...
  { ts: 't', feature: 'bash', action: 'not-asked', reason: 'file-write', session: 's1', tool_use_id: 'c', command: 'rm x' },
  { ts: 't', feature: 'bash', action: 'fast-lane', session: 's1', tool_use_id: 'd', scores: { read_only: 0.97, dev_task: 0.02, unsafe: 0.02 }, ms: 300 },
...
test('tally counts per session, old and new action names alike', () => {
  const t = tally(parseLog(log), 's1');
  assert.deepEqual(t, { allowed: 2, passed: 1, passthrough: 1, blocks: 1, agentDenies: 0 });
...
test('statusText is compact and empty when nothing happened', () => {
  ...
  assert.equal(statusText({ allowed: 3, passed: 1, passthrough: 9, blocks: 0, agentDenies: 0 }, 2), 'jev ✓3 fast-lane · ↷1 unsure · ⇊2 compact');
});
...
test('bashRowText only for judged commands', () => {
  ...
  assert.match(bashRowText(b!)!, /unsure/);
